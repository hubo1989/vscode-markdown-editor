...
#### 📤 **导出功能**
- **🖨️ PDF 导出** - 导出为 PDF 格式，完整保留样式和布局
- **📄 Word 导出** - 导出为 Word 文档，嵌入图片和完整格式
- **🌐 HTML 导出** - 生成独立 HTML 文件，包含交互式图表和公式
- **🎨 表格边框支持** - 完整的表格样式和边框格式
- **📊 图表导出** - 所有图表（Mermaid、PlantUML、ECharts）在导出文档中正确渲染
- **🖼️ 图片导出** - 图片嵌入并在所有导出格式中保持不变
